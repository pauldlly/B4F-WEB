...
> {
  ...
  let query =
    supabase
      ...
      /*
       * Seulement les événements actifs.
       */
      .eq(
        "status",
        "active",
       ...
       * les événements réservés aux packs.
       */
      .eq(
        "is_visible_only_in_packs",
        false,
  ...
  const items =
    rawRows
       ...
       * même si le filtre Supabase
       * change un jour, on ne renvoie
       * jamais un event app-only.
       */
      .filter(
        (
          row,
...
> {
  ...
  const {
    ...
    await supabase
      ...
      /*
       * IMPORTANT :
       * interdit également l'accès
       * direct par URL à un event
       * réservé uniquement à l'app.
       */
      .eq(
        "is_visible_only_in_app",
        false,
  ...
  if (
    ...
    const {
      ...
      await supabase
         ...
         * IMPORTANT :
         *
         * Même lorsqu'un événement est
         * à l'intérieur d'un pack,
         * s'il est visible uniquement
         * dans l'app, le site ne doit
  ...
  return webPackRows
    .map(
      (
         ...
         * - suspendu,
         * - passé,
         * - ou visible uniquement dans l'app,
         *
